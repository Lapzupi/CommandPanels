    ...
    public class BlockSubCommand extends BaseCommand {
        ...
        public void onAdd(final Player sender, final String panelId) {
            if (Objects.requireNonNull(plugin.getDefaultConfig().getConfig().getString("config.panel-blocks")).equalsIgnoreCase("false")) {
                sender.sendMessage(plugin.tex.colour(plugin.getTag() + ChatColor.RED + "Panel blocks disabled in config!"));
                return;
            }
            ...
            if (blockType.getType() == Material.AIR) {
                sender.sendMessage(plugin.tex.colour(plugin.getTag() + ChatColor.RED + "Look at a block to add a panel!"));
                return;
            }
            ...
            sender.sendMessage(plugin.tex.colour(plugin.getTag() + ChatColor.WHITE + panelId) + ChatColor.GREEN + "will now open when right clicking a block in the coordinates " + ChatColor.WHITE + coordinates);
        }
        ...
        public void onRemove(final Player sender) {
            ...
            sender.sendMessage(plugin.tex.colour(plugin.getTag() + ChatColor.GREEN + "Panel has been removed from block."));
        ...
        public void onList(final Player sender) {
            if (!plugin.getBlockConfig().getConfig().contains("blocks")) {
                sender.sendMessage(plugin.tex.colour(plugin.getTag()) + ChatColor.RED + "No panel blocks found.");
                return;
            }
            ...
            sender.sendMessage(plugin.tex.colour(plugin.getTag()) + ChatColor.DARK_AQUA + "Panel Block Locations:");
